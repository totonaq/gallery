	...
	let methods = {
		...
		setRespProp: function() {
			for (let i = 0; i < propArr.length; i++) {
				...
				(function resp(item) {
					if (item <= last - 1) {
						if (allLargeImg[i].width() >= keys[item] && allLargeImg[i].width() < keys[item + 1]) {
		...
		setThumbnailsWrapWidth: function() {
			for (let i = 0; i < propArr.length; i++) {
				allThumbs[i].width(allLargeImg[i].width() / propArr[i].items * galItemLength[i]);
			}
		...
		setThumbnailsDimensions: function() {
			for (let i = 0; i < propArr.length; i++) {
				allGalItems[i].css({
					'width':allLargeImg[i].width() / propArr[i].items,
